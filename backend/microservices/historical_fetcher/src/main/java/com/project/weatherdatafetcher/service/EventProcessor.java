...
import com.project.weatherdatafetcher.dto.ArchiveResponse;
import com.project.weatherdatafetcher.dto.Coordinates;
import com.project.weatherdatafetcher.dto.InputEvent;
import com.project.weatherdatafetcher.dto.OutputReceipt;
import com.project.weatherdatafetcher.model.DailyData;
...
public class EventProcessor {
        ...
        try {
            ...
            Flux.fromIterable(targetWmoIndexes)
                    .flatMapSequential(index -> webClient.get()
                            .uri(coordinatesApi + "/wmo-indexes/{wmo_index}", index)
                            .retrieve()
                    ...
                    .collectList()
                    .flatMap(coordinatesList -> fetchArchiveBatch(coordinatesList, dt_date_from, dt_date_to))
    ...
    // one archive result per coordinate, in the same order - a single batch
    ...
    private Mono<List<CoordinateAndArchive>> fetchArchiveBatch(
        ...
        return webClient.get()
                ...
                .map(body -> {
                    ...
                    List<ArchiveResponse> responses = node.isArray()
                            ? objectMapper.convertValue(node, new TypeReference<List<ArchiveResponse>>() {})
                            : List.of(objectMapper.convertValue(node, ArchiveResponse.class));
                    ...
                    List<CoordinateAndArchive> pairs = new ArrayList<>(n);
                    for (int i = 0; i < n; i++) {
                        pairs.add(new CoordinateAndArchive(resolvable.get(i), responses.get(i)));
                    }
                    return pairs;
                });
    }

    private record CoordinateAndArchive(Coordinates coordinates, ArchiveResponse archive) {}

    private List<ApiResponse> convertToApiResponses(List<CoordinateAndArchive> pairs) {

        Map<LocalDate, List<Measurement>> measurementsByDate = new TreeMap<>();

        for (CoordinateAndArchive pair : pairs) {
            Coordinates coords = pair.coordinates();
            ArchiveResponse archive = pair.archive();

            if (archive == null || archive.getDaily() == null || archive.getDaily().getTime() == null) {
                continue;
            }

            DailyData daily = archive.getDaily();
            List<LocalDate> dates = daily.getTime();

            // Open-Meteo has no concept of station name/country - backfilled
            // from regions_api's combined lookup where available, otherwise
            // left blank (matches ForecastFetcher's existing convention).
            String name = coords.getName() != null ? coords.getName() : "";
