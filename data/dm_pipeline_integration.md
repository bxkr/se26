...
**Важно:** `postgres:5432` — это **временный стенд** со схемой `weather_actual` (та же DDL, что в `infra/postgres/init.sql`), поднятый только для тестирования этого среза пайплайна, а не боевая БД `etl_service` (тот пока живёт в локальном `docker-compose`, на кластере не задеплоен). Когда `etl_service` тоже будет задеплоен в `dm-pipeline` (или другой namespace на этом кластере) — Spark-джобу нужно перенаправить на его Postgres через `helm upgrade airflow --set weatherPostgres.host=<новый-host>` (см. `backend/deploy/helm/airflow/values.yaml`, секция `weatherPostgres`).
